...
- Bare keywords / reminder text / procedural riders are never axes;
  killed keyword mechanics go to the pipeline repo's
  `mtjawnny-pipeline/docs/KEYWORD-LEDGER-CANDIDATES.md` (NOT this repo's
  gitignored docs/ — that doc must be durably version-controlled) in the
  same commit set.
