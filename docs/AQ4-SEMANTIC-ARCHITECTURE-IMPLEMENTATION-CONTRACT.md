# AQ4 PAUSED — NOT CURRENT ROUTING

**Current task authority is GitHub Issue #1: latest `K` -> active `T`. AQ4 is PAUSED.**

Frozen contract body: `benchmarks/aq4/docs/AQ4-SEMANTIC-ARCHITECTURE-IMPLEMENTATION-CONTRACT.md` (original blob `92e254b222c76f4570d0870b371bfd371093a4c0`, preserved byte-for-byte).

The frozen payload remains the durable pre-registered AQ4 benchmark/design contract and provenance record; this classification does **not** withdraw, ratify, amend, or resume its benchmark commitments. It removes only the stale current-entry-point routing claim from the live surface. If the Captain later reauthorizes AQ4, the frozen contract is evidence/input to that separately authorized task; it is not self-executing authority.

AQ4 production architecture remains unratified. No schema change, semantic retrofit, candidate component, benchmark run, Bridge v0 work, or Step6 work is authorized here.

## Registry-preserved historical reference lines

The exact original source lines below are retained mechanically so pausing the routing surface does not fabricate ruling-registry loss.

```text
  `B-MIGRATION-DISCOVERY.md` §11, verbatim text + amendments A1–A4.
header per ratified A4; derived-and-materialized in generated artifacts,
```
