...
| `door-unlocked-trigger` | a Room's door is UNLOCKED — *"when you unlock this door"*. **CR 709.5h is the trigger rule verbatim**: *"Some abilities trigger when a player unlocks a particular half of a permanent. These abilities trigger when that permanent is given the appropriate unlocked designation, **regardless of whether it was given that designation while entering the battlefield or after entering the battlefield**."* Captain-ratified 2026-08-07 (W3 sheet D8a, item 1). CR 709.5j supplies the printed noun — *"Some cards refer to a 'door' of a Room permanent. A door is a half of that permanent"* — so `door` is the CR's own word, not a coinage. **`mark`, not `msub`: the subject is the acting PLAYER** (*"when YOU unlock"*), as with `draw-trigger` on CR 121.1; whose Room it is is a §6 scope slot. **NOT `etb`, and 709.5h says so in its own sentence** — a door unlocked as the permanent enters fires the same trigger, so folding it into `etb` would split one mechanism across two tokens by an accident of timing. **The unlock COST (CR 116.2m / 709.5e) is a different thing and is not this token**: *"Unlock costs you pay cost {1} less"* (Inquisitive Glimmer) is a static, and `{T}: Lock or unlock a door of target Room` (Marina Vendrell) is CR 602 `activated` — both are the unlock as an EFFECT, and CR 113.3c keeps an effect out of the delivery slot. Measured 2026-08-09: **30 lines / 26 cards**, all previously `unclassified-trigger` | 709.5h, 709.5j |
| `day-night-changed-trigger` | the game's day/night designation CHANGES — *"whenever day becomes night or night becomes day"*. **CR 731.1a supplies both printed phrases and defines them as one event**: *"The phrases 'day becomes night' and 'night becomes day' refer to the game losing the first designation and gaining the second one."* Captain-ratified 2026-08-07 (W3 sheet D8a, item 2) — the werewolf archetype. **ONE token, not two, and the corpus settles it**: all 10 lines print BOTH directions in a single clause, so no card can ever join one side without the other and a split would produce two axes with identical membership (design goal #1). **`mark`, not `msub`: CR 731.1 makes the subject the GAME ITSELF** — *"day and night are designations that the GAME can have"* — so there is no permanent for §2a to prefix, as with `state-trigger`. **The rule is CR 731, NOT CR 728** (which is Rad Counters in the vendored snapshot); the W3 sheet's `728.1` was a wrong anchor and is corrected here. **"It becomes day" as an EFFECT is not this token** — CR 113.3c keeps it out, and the corpus proves the cut is load-bearing: Angel of Eternal Dawn (`etb`), The Celestus (`activated`) and Tovolar (`upkeep-trigger`) all print the phrase in their effect and are correctly routed elsewhere. **The 10 `If it's neither day nor night, it becomes day AS THIS CREATURE ENTERS` lines are CR 614.1c replacements, not this token**, and their staying unrouted is a separate open gap | 731.1a |
| `coin-flip-won-trigger` | a player WINS a coin flip — *"whenever you win a coin flip"*. **CR 705.2 defines the outcome and closes the set at two**: *"the player that flips the coin calls 'heads' or 'tails'. If the call matches the result, the player **wins the flip**. Otherwise, the player **loses the flip**."* Captain-ratified 2026-08-07 (W3 sheet D8a, item 3) — the Krark archetype. **This is the RESULT, never the flip as an EFFECT**, and the corpus makes the cut load-bearing: 103 of the 110 lines printing `flip` carry it in their effect and are already correctly routed by their own event (`attack-trigger`, `etb`, `cast-trigger`, `is-dealt-damage-trigger`, …). CR 113.3c decides it. **`mark`, not `msub`: CR 705.2's subject is a PLAYER** (*"only the player who flips the coin wins or loses the flip"*), so scope is a §6 slot — `you` 4 · `a player` 2. **A `coin counter` is not a coin flip** — Athreos, Shroud-Veiled prints the words and is a counter type (CR 122), correctly excluded. Measured 2026-08-09: **6 lines / 6 cards** | 705.1, 705.2 |
| `coin-flip-lost-trigger` | the other half of CR 705.2's two-outcome enumeration — *"whenever you lose a coin flip"*. Captain-ratified 2026-08-07 (W3 sheet D8a, item 3). **1 line** (Karplusan Minotaur), and it is written as its own row for the same reason `noncombat-damage-to-planeswalker` is: **a token RESERVED BY A CR ENUMERATION with its emitter present**, not a token invented for one card. **NOT folded into `coin-flip-won-trigger`, and the D3f test is what forbids it** — winning and losing are opposite outcomes of the same flip, so the distinction changes WHETHER the effect happens, not how much. Karplusan Minotaur proves a card can carry both as two separate abilities and the two do different things, which is exactly the split §8b preserved for counters | 705.2 |
| `exiled-from-battlefield-trigger` | a permanent LEAVES the battlefield FOR EXILE — *"whenever a creature is exiled from the battlefield"*. Captain-ratified 2026-08-07 (W3 sheet D8a, item 4). **CR 400.1 makes exile a zone like any other**, and §2 had named the graveyard destination (`to-graveyard-from-anywhere` and its three siblings, CR 700.4) while leaving this one unnamed — the same asymmetry `leaves-graveyard-trigger` closed in the other direction. **NOT `leaves-battlefield-trigger`, on the ratified D3f test**: a creature that DIES does not trigger Soulherder, so the destination changes WHETHER the effect happens, not how much. The blink archetype is the deck this token names. Measured 2026-08-09: **3 lines** (Soulherder, Captain Marvel Shooting Star, Market Gnome) — the W3 sheet's estimate of 12 counted the EFFECT-side `if a card is exiled this way` lines, which CR 113.3c keeps on their own triggers and which the diff confirms do not move | 400.1, 700.4 |
| `player-loses-game-trigger` | a player LOSES THE GAME. **CR 603.9 gives the shape its own rule and states it verbatim**: *"Some triggered abilities trigger specifically **when a player loses the game**. These abilities trigger when a player loses **or leaves** the game, regardless of the reason, unless that player leaves the game as the result of a draw."* Captain-ratified 2026-08-07 (W3 sheet D8a, item 5). **The sheet called this one marginal for deck-building and that assessment stands** — it is recorded here rather than argued up, because Captain's ratified criterion is deck-building relevance and honesty about a weak case is worth more than a padded one. It earns a row on the CR having named it, not on its archetype. **`mark`, not `msub`: 603.9's subject is a PLAYER**, so scope is a §6 slot (`a player` 3 · `another player` 1 · `enchanted player` 1). **"You lose the game" as an EFFECT is not this token** — Immortal Coil and Transcendence print it and are correctly `state-trigger` (CR 603.8), which is the same CR 113.3c cut every row in this block turns on. Measured 2026-08-09: **7 lines / 7 cards** — an earlier draft of this row said 5, taken from a probe that required the line to START with `when` and so lost the CR 207.2c ability-word prefix (`Burning Chains — When the chosen player loses the game`) and a compound (`…AND whenever an opponent loses`). Corrected by `foundry_recorded_numbers.py` | 603.9 |
