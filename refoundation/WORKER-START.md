...
- **Live task authority is GitHub Issue #1**: the latest accepted `K` names the
  active `T`. Nothing else assigns you work.

## What a fresh session does

1. Inspect local state before mutating anything.
2. Read Issue #1 — latest accepted `K`, then its active `T`.
