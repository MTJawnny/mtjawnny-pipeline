"""The ONE reader of the Comprehensive Rules — L2, and the bottom of the CR seam.

## Why this module exists

The CR is parsed at run time, so its markup is a live dependency of every
derivation in the repository. Reading it through anything but this module
reintroduces the recorded failure where the bold rule markers (`**605.1a.**`)
make every enumeration return empty: `path.read_text()` on a CR is a defect, and
`edition.text()` is the whole of the cure.

## What moved here in S6, and what did not

The reusable CR semantics moved: edition selection and `MTJ_CR_PATH`, the markup
vocabulary and required anchors, the declared encoding-damage register, the
pure-deletion normalization law, encoding detection plus mechanical repair, the
parseability assertions, and the read/cache API.

The operator half did NOT: `--report`, the CLI, and the negative-control
fixtures stay in `experiments/foundry_cr.py`, which is now a shell over this
module. `experiments/foundry_cr_edition_diff.py` is later operator work and is
untouched by S6.

## The halt boundary is NOT imported upward

Legacy code reaches the CR through `fc.halt`, which prints `STOP — …` and exits
the process. A library may not exit a process it does not own, and L2 may not
import `foundry_common` at all -- so every former `fc.halt(...)` here RAISES
`CRError` instead, and the legacy shell converts it back to the historic
process boundary. Same message, same stop, decided one layer up.

## Layer law

`mtg/cr/**` never imports, calls or references `mtg/shapes/**`, including inside
a function body. This module imports stdlib and `mtj_foundry.paths` only.
"""
...
class CRError(RuntimeError):
    """A CR the pipeline refuses to proceed on.

    Carries the exact message the legacy `fc.halt` printed, so the shell can
    reproduce the historic `STOP — …` boundary verbatim.
    """
...
def _demojibake(s: str) -> str:
    ...
    try:
        ...
        raise CRError(f"cannot reverse the encoding damage in {s!r}: {exc}. This is "
...
def _repair_encoding(norm: str, path: Path) -> str:
    ...
    for rule, decl in _KNOWN_ENCODING_DAMAGE.items():
        ...
        for corrupt, (fixed, _n) in decl["repairs"].items():
            ...
            if derived != fixed:
                raise CRError(f"the repair derived for {corrupt!r} is {derived!r}, "
...
def _assert_parseable(norm: str, path: Path) -> None:
    ...
    if re.search(r"(?m)^\*\*\d", norm):
        ...
        raise CRError(f"bold rule markers survived normalization of {path}: "
...
def text(path: Path) -> str:
    ...
    if key not in _cache:
        if not path.exists():
            raise CRError(f"Comprehensive Rules not found at {path}. This repo "
        ...
        if _MOJIBAKE.search(norm):
            ...
            raise CRError(f"encoding damage survived the repair pass in {path}: "
...
def effective_date(txt: str = None) -> str:
    ...
    if not m:
        raise CRError("CR has no 'effective as of <date>' sentence — cannot version "
