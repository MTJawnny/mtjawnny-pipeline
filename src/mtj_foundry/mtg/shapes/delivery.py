"""DELIVERY / ability-shape derivation — L2, the MTG shapes substrate.

## What this owns

The reusable half of the legacy DET ability-shape extractor: the CR- and
grammar-derived decomposition of a card into ability lines, and the structural
naming of each line's §2 DELIVERY slot. It judges nothing and writes nothing —
a shape with no RATIFIED token is reported as `UNRATIFIED:<descriptor>`, never
approximated onto the nearest ratified one.

It also owns the keyword→home derivation that R6 moved here:
`find_home`, `keyword_homes()` and `build_keyword_homes`. They consume
`parse_delivery`, so the CR half may not own them — placing them under
`mtg/cr/**` is exactly the import cycle R6 was written to remove, and the
legacy `_twin` cross-instance state sync existed only because of it. That
workaround is gone: the derived state has ONE owner and therefore one instance.

`keyword_homes()` is the SINGLE owner of the CR-class fallback. Live source
implemented that rule twice — once in the operator report and once in
`build_keyword_homes` — and `build_keyword_homes` now consumes the one
derivation instead of re-deciding it.

## The three-stage build order, which is a contract and not an accident

1. delivery parsing **without** the keyword map. CR-templated text classifies
   on its own branches; `keyword_line_tokens` returns `[]` while
   `KEYWORD_HOME is None`, and `keyword_form_tokens` returns `[]` while
   `KEYWORD_FORMS` is falsy. They are INERT, deliberately. (Both return the
   empty list. An earlier description of this seam said the second returned
   `None`; the code says otherwise and the code is the fact.)
2. `build_keyword_homes` derives the map using stage 1.
3. delivery parsing **with** the map available.

Every consumer performs that sequence. Skipping a step is a recorded defect
class, not a hypothetical one.

## Layer law

Imports stdlib, `mtj_foundry.mtg.cr.edition` and `mtj_foundry.mtg.cr.keywords`
only. Never `mtg/shapes -> mtg/cr -> ...` in reverse, never `experiments`,
never a process exit: this module RAISES `ShapeError` and the legacy shell
re-establishes the historic `STOP — …` halt boundary.

## It derives no repository root, and it reads no repository file by itself

Every repository-owned input arrives explicitly from a composition boundary:
the CR edition path, the grammar path, the CR-check registry path, and the
card-text rules below. There is no `__file__` ancestry, no cwd, no `.git`
search, no module-level ROOT, and no import-time artifact read. Installed into
an arbitrary `site-packages`, this module still imports and still has no
opinion about where a repository is.
...
class ShapeError(RuntimeError):
    """An ability-shape derivation the pipeline refuses to proceed on.

    The legacy boundary converts this into the historic `STOP — …` process
    contract. A library may not exit a process it does not own, and losing a
    raised error would be a truth change rather than a plumbing change.
    """


class CardTextRules:
    """The card-text primitives this substrate consumes but does not own.
    ...
    REQUIRED = frozenset({"full_oracle_text", "canonicalize_self_reference",
                         "is_mode_line", "modal_header_re",
                         "roll_instruction_re", "die_row_re"})
...
# THE DERIVED, ORDER-DEPENDENT MODULE STATE, named in one place.
#
# Every name here is assigned by a `global` statement in a build step below and
# is INERT until that step runs. The legacy boundary re-exports these by live
# reference rather than by value: a copy taken at import time would be the
# second-module hazard again, one layer over.
DERIVED_STATE = (
    "CR_ABILITY_WORDS", "CR_CREATURE_TYPES", "CR_DAMAGE_RECIPIENTS",
    "CR_KEYWORD_NAMES", "CR_ZONES", "KEYWORD_FORMS", "KEYWORD_HOME",
    "LANDWALK_BASES", "LANDWALK_MODIFIERS", "SELF_NOUN_RX", "TRIGGER_VERB",
    "TYPECYCLING_WORDS",
)


_CARD_TEXT_RULES: CardTextRules | None = None


def use_card_text_rules(rules: CardTextRules) -> None:
    """Install the injected card-text rules. Called by the boundary, once."""
    global _CARD_TEXT_RULES
    if not isinstance(rules, CardTextRules):
        raise ShapeError(f"card-text rules must be a CardTextRules, got "
                         f"{type(rules).__name__}")
    _CARD_TEXT_RULES = rules


def _rules() -> CardTextRules:
    if _CARD_TEXT_RULES is None:
        raise ShapeError(
            "card-text rules have not been installed. This substrate receives "
            "them from its composition boundary (`use_card_text_rules`); it "
            "does not go looking for a repository to read them out of.")
    return _CARD_TEXT_RULES
...
def build_self_noun_rx(cards: dict, cr_path) -> None:
    ...
    build_cr_enumerations(cr_path)
    vocab = _keywords.type_vocabulary(cr_path)
    ...
    for key in ("card_types", "subtypes"):
        ...
        if missing:
            raise ShapeError(f"Self-reference noun set is missing CR 205 {key}: "
    ...
    if "equipment" not in nouns or "siege" not in nouns:
        raise ShapeError("Self-reference noun set has no 'equipment'/'siege' — the "
    ...
    if unknown:
        raise ShapeError(f"Corpus type lines print subtypes CR 205.3 does not "
...
# ---------------------------------------------------------------------------
def ratified_delivery_tokens(grammar_path) -> dict:
    ...
    if not grammar_path.exists():
        raise ShapeError(f"{grammar_path} not found — the ratified DELIVERY "
                         f"vocabulary lives there")
    ...
    if not m:
        raise ShapeError("CODEBOOK-NAMING-GRAMMAR.md: could not locate §2's DELIVERY table. "
    ...
    for row in m.group(1).splitlines():
        ...
        if re.fullmatch(r"[a-z0-9\-]+", tok):
            tokens[tok] = cells[-1]
    if len(tokens) < 10:
        raise ShapeError(f"parsed only {len(tokens)} DELIVERY tokens from §2 — expected ~19. "
                         f"Refusing to run with a truncated vocabulary.")
    return tokens


def cr_action_terms(checks_path) -> dict:
    if not checks_path.exists():
        raise ShapeError(f"{checks_path} not found — generate the CR check "
                         f"registry before deriving trigger verbs")
...
def ability_lines(card: dict) -> list:
    ...
    txt = strip_reminder(_rules().full_oracle_text(card))
...
def deliveries_for_lines(card: dict, ratified: dict):
    ...
    for i, line in enumerate(lines):
        ...
        if is_roll and (_rules().die_row_re.match(s) or _rules().is_mode_line(line)):
            yield line, [(t, f"die-row:{d}") for t, d in header_parsed]
            continue
        if _rules().is_mode_line(line):
        ...
        is_modal = bool(_rules().modal_header_re.search(s))
        ...
        is_roll = bool(_rules().roll_instruction_re.search(line)) and (
            bool(_rules().die_row_re.match(nxt))
            or (not is_modal and i + 1 < len(lines)
                and _rules().is_mode_line(lines[i + 1])))
        yield line, parsed


CR_DAMAGE_RECIPIENTS = None    # CR 120.1, parsed by build_cr_enumerations()
CR_ZONES = None                # CR 400.1, parsed by build_cr_enumerations()
CR_CREATURE_TYPES = None       # CR 205.3m, parsed by build_cr_enumerations()
...
def build_cr_enumerations(cr_path) -> None:
    ...
    lesson, 2026-08-05)."""
    global CR_DAMAGE_RECIPIENTS, CR_ZONES, CR_ABILITY_WORDS, CR_CREATURE_TYPES
    ...
    txt = _edition.text(cr_path)

    def enum(rx, what, must):
        m = re.search(rx, txt, re.M)
        if not m:
            raise ShapeError(f"Could not parse {what} from the CR. The wording has "
        ...
        if missing or any(v.startswith("and ") for v in vals):
            raise ShapeError(f"CR parse of {what} is incomplete or malformed: "
    ...
    if not m:
        raise ShapeError("Could not parse CR 207.2c's ability-word list. The wording "
    ...
    if missing or any(w.startswith("and ") for w in words):
        raise ShapeError(f"CR 207.2c ability-word parse is incomplete or malformed: "
                         f"missing={missing} got={sorted(words)}")
    CR_ABILITY_WORDS = words

    # CR 205.3m's creature types, parsed HERE rather than re-parsed inside
    # `_damage_recipient`. CR 109.2 makes a bare creature type denote a
    # creature permanent, and that read runs once per damage clause -- so the
    # vocabulary is derived once, beside the other three CR enumerations, and
    # under the same guard: `_damage_recipient` already refuses to run when
    # these have not been built.
    CR_CREATURE_TYPES = _keywords.type_vocabulary(cr_path)["creature_types"]
...
def _damage_recipient(tail: str, card: dict = None):
    ...
    if CR_DAMAGE_RECIPIENTS is None:
        raise ShapeError("CR 120.1 recipients not built — call build_cr_enumerations() "
...
def in_card_name(line: str, pos: int, card: dict) -> bool:
    ...
    The injected self-reference canonicaliser normally collapses the name to
    `~` before this point and would have prevented the question; it does not
    fire on these because the colon breaks its matching. That is the root cause
    and it lives in a shared ratified helper one layer up, so it is reported
...
def build_trigger_verbs(actions: dict) -> None:
    ...
    if not {"discard", "sacrifice", "mutate", "transform",
        ...
        raise ShapeError("Trigger-verb vocabulary lost a known CR keyword action — "
...
# INERT UNTIL BUILT, and that is the contract, not an accident. The legacy
# shell builds this at ITS import time, from the CR-check artifact it locates;
# an installed consumer calls `build_trigger_verbs(cr_action_terms(path))`
# itself. A library may not go looking for a repository to read.
...
# ---------------------------------------------------------------------------
# R6 — keyword -> §2 DELIVERY home. ONE owner, and it lives HERE.
# ---------------------------------------------------------------------------
# `find_home` consumes `parse_delivery`, so the CR half cannot own it without
# re-creating the cycle. `keyword_homes()` is the lift out of the legacy
# operator report, and it is the SINGLE place the CR-class fallback is decided:
# live source implemented that one rule twice, in the two modules on opposite
# ends of that cycle.

# The report spelling for a keyword whose CR ability class gives it NO §2
# DELIVERY slot at all. It is a HOME, not a token: `build_keyword_homes` must
# never route a line onto it, which is why the record carries `delivery` and
# `home` separately.
NO_DELIVERY_HOME = "(none — spell ability)"
CR_CLASS_VIA = "CR ability class (702.Na)"
...
def find_home(kw: dict, ratified: dict) -> tuple:
    ...
    for letter in sorted(kw["subrules"]):
        m = _keywords.MEANS.search(kw["subrules"][letter])
        if m:
            text = m.group(1).strip()
            break
    if _keywords.effective_classes(kw) == ["activated"]:
        if "activated" not in ratified:
            raise ShapeError(
                "grammar §2 no longer ratifies the `activated` DELIVERY "
    ...
    tok, desc = parse_delivery(text, ratified, None)
    return tok, desc, text


def keyword_homes(keywords: dict, ratified: dict) -> dict:
    """{cr_number: home record} for every named CR 702 keyword.
    ...
    them appropriate homes.'* Right, and it makes the 44-triggered-keyword
    'gap' mostly illusory: a triggered keyword does not need NEW delivery
    vocabulary, it needs to be routed to the token its own CR templated text
    already resolves to.

    **This is the single owner of the CR-class fallback.** When the templated
    text parses to no ratified token, the keyword's own CR ABILITY CLASS is
    consulted -- §2 already has a slot for it, so a static keyword's home is
    `static`, and that is a real home, not a gap. Only a keyword whose class
    ALSO gives no slot is reported unresolved, never approximated.

    `home` and `delivery` are deliberately two keys. A CR `spell` keyword has a
    HOME -- the declared no-slot representation -- and no DELIVERY token, and
    collapsing them would route corpus lines onto a label that §2 does not
    ratify as a token. `home_via`, `home_descriptor`, `cr_text` and
    `unresolved_reason` carry what the operator report prints.
    """
    out = {}
    for num in sorted(keywords):
        kw = keywords[num]
        if not kw["name"] or num == _keywords.PREAMBLE_RULE:
            continue
        tok, desc, text = find_home(kw, ratified)
        rec = {"cr": f"702.{num}", "keyword": kw["name"],
               "home": tok, "delivery": tok,
               "home_descriptor": desc, "cr_text": text,
               "home_via": None, "unresolved_reason": None}
        if tok is None:
            eff = _keywords.effective_classes(kw)
            if eff == ["static"] and "static" in ratified:
                rec["home"] = rec["delivery"] = "static"
                rec["home_via"] = CR_CLASS_VIA
            elif eff == ["spell"]:
                # A home, NOT a token. `delivery` stays None on purpose.
                rec["home"] = NO_DELIVERY_HOME
                rec["home_via"] = CR_CLASS_VIA
            elif text is None:
                rec["unresolved_reason"] = \
                    "no templated text AND no single-class fallback"
            else:
                rec["unresolved_reason"] = \
                    f"templated shape has no ratified token: {desc}"
        out[num] = rec
    return out


def build_keyword_homes(ratified: dict, cr_path) -> None:
    """keyword name -> §2 DELIVERY token, derived from the CR (§2b).

    Stage 2 of the three-stage build. `CR_KEYWORD_NAMES` is set BEFORE the
    homes are derived, because `ability_word_prefix`'s CR 702.Na refusal reads
    it while `find_home` parses the templated text -- and the two facts are not
    one fact: a keyword whose home cannot be derived is absent from the map and
    still a keyword.
    """
    global KEYWORD_HOME, CR_KEYWORD_NAMES
    kws = _keywords.load_702(cr_path)
    CR_KEYWORD_NAMES = {kw["name"].lower() for num, kw in kws.items()
                        if kw["name"] and num != _keywords.PREAMBLE_RULE}
    if not {"awaken", "impending", "ward"} <= CR_KEYWORD_NAMES:
        raise ShapeError("CR 702 keyword-name parse lost a keyword that prints "
                         "an em-dash parameter; the ability-word strip would "
                         "then read it as a flavor word. got "
                         f"{len(CR_KEYWORD_NAMES)} names.")
    homes = {}
    for _num, rec in keyword_homes(kws, ratified).items():
        if rec["delivery"]:
            homes[rec["keyword"].lower()] = rec["delivery"]
    if "battle cry" not in homes:
        raise ShapeError("Keyword home map has no 'battle cry' — the CR 702 "
                         "parse failed. Refusing to run with a partial keyword "
                         "vocabulary.")
    if homes.get("equip") != "activated":
        raise ShapeError("Equip is not routing to `activated`. CR 702.6a states "
                         "it outright and §2b quotes that rule as its worked "
                         "example; a map that loses it is not a §2b router.")
    KEYWORD_HOME = homes
    build_landwalk_template(cr_path)
    build_keyword_forms(kws)
...
def build_landwalk_template(cr_path) -> None:
    ...
    v = _keywords.type_vocabulary(cr_path)
    ...
    if not {"swamp", "island", "land"} <= LANDWALK_BASES:
        raise ShapeError(f"CR 205.3i parse lost a basic land type: {sorted(LANDWALK_BASES)}")
    ...
    for probe in ("plains", "mountain", "basic", "land", "sliver"):
        if probe not in TYPECYCLING_WORDS:
            raise ShapeError(f"CR 205 parse lost {probe!r}, which CR 702.29e's typecycling "
...
def build_keyword_forms(kws: dict) -> None:
    ...
    for num, kw in kws.items():
        ...
        for f in cr_printed_forms(kw):
            ...
            try:
                ...
                raise ShapeError(f"CR 702 printed form {f!r} for {kw['name']} did not "
    ...
    for anchor in ("ward", "equip", "craft", "cumulative upkeep", "champion"):
        if anchor not in forms:
            raise ShapeError(f"No CR printed form derived for {anchor!r}. The 702 parse "
...
def parse_deliveries(line: str, ratified: dict, card: dict = None) -> list:
    ...
    if card is not None:
        body = _rules().canonicalize_self_reference(body, card)
    low = body.lower()
...
def parse_delivery(line: str, ratified: dict, card: dict = None) -> tuple:
    ...
    is_mode = _rules().is_mode_line(raw)
