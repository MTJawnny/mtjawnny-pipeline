#!/usr/bin/env python3
"""Assembles the batch-7 seed for the T3 Axis Foundry per MASTER-HANDOFF.md
section 5 (batch-seed method) with section 5.6 targeting, per
TRIAGE-BATCH-6.md's D1-D8/section-11 ratifications: confirmation targets
for batch-6's three new thin captain-authored axes (all n<=2:
activated-tap-or-untap-any-permanent, leaves-battlefield-trigger-create-
token-mutagen, activated-untap-target-creature), reinforcement for two
axes that lost their sole batch-6 example to Gate #0 or an M8-driven
rehome and are back down to a single member
(activated-tap-or-untap-any-creature/Puppet Strings,
creates-token-with-x-scaled-counters/Wild Hypothesis), and continued
zero-hit carries. Deliberately NOT hand-searched this batch: the three
energy/battle-damage axes explicitly registered as "membership filled by
derivation at the full-corpus pass" in their own codebook definitions
(rule:targeted-battle-damage, rule:gives-energy-counters-condition,
rule:energy-outlet-infinite) -- hand-picking examples for these would
contradict their own registration note.

rule:targets-highest-life-opponent searched this batch (its "targets the
opponent with highest life" pattern, as opposed to "choose"-worded or
power/toughness-scaling-by-life-difference cards) -- zero clean hits,
carried forward empty rather than force a loose match.
rule:cast-trigger-transforms-into-creature (Veiled Serpent's sole
remaining member after Village Ironsmith // Ironfang's batch-6 removal)
searched for another "transforms when an opponent casts a spell" card --
zero hits, carried forward empty.

Per batch-6 D1 (Gate #0): draws from foundry_common.load_corpus_gated(),
not load_corpus() -- nowhere-legal cards are never offered to SYNTH.

Usage: python3 experiments/foundry_assemble_batch7.py
"""
import sys
import json
import random
...
FOUNDRY_SEED = 20260730  # date of assembly, matching the seed=date-of-assembly convention
TARGET_SIZE = 1200
OUT_PATH = fc.FOUNDRY_OUT_DIR / "batch7_assembled.json"

CONFIRMATION_PICKS = {
    "rule:activated-tap-or-untap-any-permanent": ["Captain of the Mists"],
    "rule:leaves-battlefield-trigger-create-token-mutagen": ["Splinter, the Mentor"],
    "rule:activated-untap-target-creature": ["Fyndhorn Brownie", "Seeker of Skybreak"],
    "rule:activated-tap-or-untap-any-creature": ["Puppeteer", "Stonybrook Angler"],
    "rule:creates-token-with-x-scaled-counters": ["Leyline Invocation", "Fractal Summoning"],
    # Carried forward again, still zero real hits this batch's search too:
    "rule:targets-highest-life-opponent": [],
    "rule:cast-trigger-transforms-into-creature": [],
...
def main():
    cards, name_index, gated_out_count = fc.load_corpus_gated()
    ...
    b5 = json.loads((fc.FOUNDRY_OUT_DIR / "batch5_assembled.json").read_text())
    b6 = json.loads((fc.FOUNDRY_OUT_DIR / "batch6_assembled.json").read_text())
    prior_oids = (set(b1["all_oracle_ids"]) | set(b2["all_oracle_ids"]) | set(b3["all_oracle_ids"])
                  | set(b4["all_oracle_ids"]) | set(b5["all_oracle_ids"]) | set(b6["all_oracle_ids"]))
    print(f"gate-passing corpus: {len(cards)} cards ({gated_out_count} gated out by Gate #0); "
          f"prior batches (1-6) excluded: {len(prior_oids)}")
    ...
    out = {
        ...
        "gate0_gated_out_count": gated_out_count,
        "no_other_lane_reinforcement_reason": "batch 6 read all n>=3 OTHER-lane token groups in full plus a "
            "large sample of the n=2 tier (spot-checked, not exhaustively cleared -- see TRIAGE-BATCH-6.md's "
            "own methodology note) and found zero new coherent multi-card families beyond the two folded "
            "directly into axis verdicts/member_additions -- no residue left to reinforce as of batch 6.",
        "deduped_against": "all batch-1 (n=500) through batch-6 (n=1200) cards, and Gate #0 (batch-6 D1) -- "
            "the corpus pool itself excludes nowhere-legal cards before strata are built.",
