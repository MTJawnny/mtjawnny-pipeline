...
python3 experiments/aq4_benchmark/aq4_binding.py --report    # Phase-A queue
```

### The Phase-A unit-binding work queue — a sheet, never an authority

`--report` writes `experiments/out/aq4/unit-binding-workqueue.json`, following
this repository's existing working-sheet convention: **the generator is tracked
and the generated bytes are not** (`experiments/out/` is gitignored, exactly as
for `foundry_locality.py --report`). Regenerate it; never cite it, never freeze
it, never stage it.

It answers **one** question — *which existing semantic occurrence is the
intended benchmark unit for this frozen card-pair case?* — and nothing else. It
carries no participant enumeration, no correspondence, no semantic verdict and
no candidate data, and the binding validator **refuses** it as an artifact.

Derived from frozen machinery, and independently reproducing the accepted
preflight: **354** cases — **98** deterministically bound, **239** needing human
adjudication, **17** administratively not applicable; per tranche
28/71/6 · 37/84/6 · 33/84/5; **159** cases with exactly one side resolved.
Unresolved-side reason census (scoped to cases that actually need
adjudication): detector reaches 0 → 87 · 2 → 143 · 3 → 63 · 4 → 17 · 5 → 9.

**The scope of that census is load-bearing.** A side inside an administratively
not-applicable case is never adjudicated for that case, so counting its
ambiguity would inflate the queue with work nobody will do. Those **9** sides
are reported separately rather than dropped.

**It decides nothing.** Every unresolved side keeps its full lawful choice set
with no occurrence selected; the detector-reached subset sits beside it as a
hint, never a pick; every resolved side keeps the rule that resolved it. The
report pins its frozen inputs (pair-set hash, surface digest, corpus ref, CR
hash, schema versions) and **halts** rather than regenerate under a moved
benchmark state.

**Phase A is unit binding only.** Participant enumeration, correspondence and
the singleton rule come later, and the final combined binding artifact remains
the only future authority.
