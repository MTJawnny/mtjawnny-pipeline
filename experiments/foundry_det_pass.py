...
# typed refusal. S13 then moved the review orchestration -- the two command
# flows, the CLI, the NOTE lines and `det_locality_owner`'s accounting -- to
# `mtj_foundry.det_pass_job`; the reads, writes, backup and STOP stay here.
...
# S13: the two operator flows, the CLI, the NOTE lines and the locality-owner
# accounting are `mtj_foundry.det_pass_job`'s. Every path, read, backup, write
# and STOP below is still this boundary's, handed over as call-time callables.
from mtj_foundry import det_pass_job as _job  # noqa: E402
...
def load_axis_patterns():
    ...
    # is still /1.
    #
    # S11: the partition itself is `codebook_det_resolution`'s. The READ stays
    # here; S13 moved the NOTE text to `det_pass_job.axis_pattern_notes`.
    ...
    for line in _job.axis_pattern_notes(ruled_gaps, deferred_gaps, RULED_AXISLESS_PATTERNS):
        print(line)
...
def cmd_generate_samples():
    return _job.cmd_generate_samples(_context())


def new_locality_stats() -> dict:
    return _job.new_locality_stats()
...
def det_locality_owner(card, clause: str, stats: dict):
    ...
    mutation. S13: the accounting is `det_pass_job.det_locality_owner`; the
    resolver is this boundary's `foundry_locality`, looked up when called."""
    return _job.det_locality_owner(card, clause, stats, fl.resolve, fl.OWNER)


def cmd_apply(verdicts_path: str):
    return _job.cmd_apply(verdicts_path, _context())


def _context():
    return _job.DetPassContext(
        description=__doc__,
        samples_report_path=SAMPLES_REPORT_PATH,
        samples_report_md_path=SAMPLES_REPORT_MD_PATH,
        hits_cache_path=HITS_CACHE_PATH,
        codebook_path=CODEBOOK_PATH,
        load_axis_patterns=lambda: load_axis_patterns(),
        load_corpus_gated=lambda: fc.load_corpus_gated(),
        det_scan_texts=lambda card: fc.det_scan_texts(card),
        full_oracle_text=lambda card: fc.full_oracle_text(card),
        assert_lattice_invariant=lambda p: assert_lattice_invariant(p),
        expand_lattice_pattern=lambda p, cards: expand_lattice_pattern(p, cards),
        compute_special_hits=lambda slug, texts, cards: compute_special_hits(slug, texts, cards),
        compute_full_hits=lambda pattern, texts: compute_full_hits(pattern, texts),
        quote_pattern_src=lambda p: quote_pattern_src(p),
        det_locality_owner=lambda card, clause, stats: det_locality_owner(card, clause, stats),
        lattice_parent_scopes=lambda axes: lattice_parent_scopes(axes),
        halting=lambda fn, *args, **kwargs: _halting(fn, *args, **kwargs),
        stop=lambda message: fc.halt(message),
        write_json=lambda path, data: fc.write_json(path, data),
        load_codebook=lambda: fcb.load_codebook(CODEBOOK_PATH),
        corpus_ref_current=lambda: fcb.corpus_ref_current(),
        backup_codebook=lambda tag: fcb.backup_codebook(tag),
        write_codebook_atomic=lambda path, cb, label: fcb.write_codebook_atomic(path, cb, label),
    )


def main():
    _job.run(None, _context())
