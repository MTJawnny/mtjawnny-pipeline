...
1. Check whether it genuinely matches one of these existing axes' DEFINITION (not just a superficial word overlap). If it does: lane="codebook", label=that axis's EXACT slug (copy it verbatim, including the "rule:" prefix).
...
- Do NOT propose a "cantrip" axis -- rule:cantrip is a ratified deterministic predicate (any card type, mana value <=2, oracle text includes a draw-a-card rider) computed directly from card data, not something SYNTH needs to identify.
