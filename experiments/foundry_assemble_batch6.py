#!/usr/bin/env python3
"""Assembles the batch-6 seed for the T3 Axis Foundry per MASTER-HANDOFF.md
section 5 (batch-seed method) with section 5.6 targeting, per TRIAGE-BATCH-5.md
section 5's batch-6 feedback and section 10's punch list: confirmation
targets for batch-5's thin new captain-authored axes (all n=1) and the two
new OTHER-lane promotions (n=2), continued zero-hit carries
(rule:targeted-battle-damage, 4th batch running; rule:gives-energy-counters-condition
and rule:energy-outlet-infinite still thin after 3 batches), and the D16
full-corpus test-case card (Plunge into Darkness) confirmed here as a real,
corpus-valid, not-yet-reviewed card for rule:library-dig-to-hand rather than
waiting for the full pass.

rule:death-trigger-land-becomes-swamp searched again this batch (its
"target land(s) become Swamp on death" pattern is extremely narrow --
Cyclopean Titan/Giant may be the only two printings) -- zero further hits,
carried forward empty rather than force a match, same call made for
rule:targeted-battle-damage every batch since batch 3.

Usage: python3 experiments/foundry_assemble_batch6.py
"""
import sys
import json
import random
...
TARGET_SIZE = 1200
OUT_PATH = fc.FOUNDRY_OUT_DIR / "batch6_assembled.json"

CONFIRMATION_PICKS = {
    "rule:counters-target-spell": ["Statute of Denial", "Unravel"],
    "rule:etb-create-token-creature-conditional": ["Nightsquad Commando", "Extricator of Sin // Extricator of Flesh"],
    "rule:etb-create-token-mutagen": ["Michelangelo, Weirdness to 11", "Raphael, the Muscle"],
    "rule:leaves-battlefield-trigger-create-token-creature": ["Thragtusk", "Walker of the Grove"],
    "rule:death-trigger-scroll-regrowth": ["Living Lightning"],
    "rule:delayed-cantrip": ["Gravebind", "Heal"],
    "rule:etb-destroy-target-enchantment": ["War Priest of Thune", "Wispmare"],
    "rule:cant-be-blocked-by-color": ["Dauthi Horror", "Vine Mare"],
    "rule:gives-energy-counters-condition": ["Lightning Runner"],
    "rule:energy-outlet-infinite": ["Consulate Surveillance", "Plasma Caster"],
    "rule:library-dig-to-hand": ["Plunge into Darkness"],
    # Carried forward again, still zero real hits this batch's search too:
    "rule:death-trigger-land-becomes-swamp": [],
...
def main():
    ...
    prior_oids = (set(b1["all_oracle_ids"]) | set(b2["all_oracle_ids"]) | set(b3["all_oracle_ids"])
                  | set(b4["all_oracle_ids"]) | set(b5["all_oracle_ids"]))
    print(f"corpus: {len(cards)} cards; prior batches (1-5) excluded: {len(prior_oids)}")
    ...
    out = {
        ...
        "no_other_lane_reinforcement_reason": "batch 5 verified against literally all 1,248 OTHER-lane token "
            "groups (not a sample) and found zero new-family promotions beyond the two already built into "
            "codebook v0.5 -- no residue left to reinforce.",
        "deduped_against": "all batch-1 (n=500), batch-2 (n=1200), batch-3 (n=1200), batch-4 (n=1200), and "
