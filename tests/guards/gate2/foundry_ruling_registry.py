...
# S15.R2 established the real guard route for stale-registry diagnostics.
# S16.P3 closes the remaining routing debt by making the generated preamble use
# this same single command source instead of publishing the retired experiments/
# path. One route now governs diagnostics, generation instructions and --check.
...
def render_markdown(reg: dict) -> str:
    ...
    S16.P3 changes only the preamble's operator route: it now renders from
    `REGENERATE_CMD`, the same current guard command already used by stale-file
    diagnostics. Registry contents, ordering, metrics and deletion-gate meaning
    remain byte-for-byte governed by the construction below.
    """
    L: list[str] = []
    A = L.append
    A("# RATIFIED RULINGS REGISTRY — generated, do not hand-edit")
    A("")
    A(f"Generated by `{REGENERATE_CMD}`. Re-run after any")
    ...
    A("")
    A("```")
    A(f"{REGENERATE_CMD} --check <NAME>.md")
    A("```")
    A("")
    A("## Summary")
    A("")
